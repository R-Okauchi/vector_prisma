...
from prisma.cli._node import npm, node
...
def run(
    ...
    log.debug('Running vector prisma command with args: %s', args)
    ...
    if args and args[0] in {'--help', '-h'}:
        ...
        click.echo('  ' + 'For Vector Prisma Client Python commands run ' + click.style('vector-prisma py --help', bold=True))
...
def ensure_cached() -> CLICache:
    ...
    if not entrypoint.exists():
        click.echo('Installing Vector Prisma CLI')
